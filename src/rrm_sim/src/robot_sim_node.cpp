...
public:
  RobotSimNode()
    ...
    subscription_ = this->create_subscription<rrm_msgs::msg::Command>(
        "move_command", 10,
        std::bind(&RobotSimNode::cmd_callback, this, std::placeholders::_1));

    service_ = this->create_service<rrm_msgs::srv::Command>(
        "move_command",
        std::bind(&RobotSimNode::cmd_service_callback, this,
...
private:
  ...
  void cmd_service_callback(
      const std::shared_ptr<rrm_msgs::srv::Command::Request> request,
      std::shared_ptr<rrm_msgs::srv::Command::Response> response) {
    if (request->positions.size() != joint_names_.size()) {
      response->result_code = 1;
      response->message = "Received an incorrect size of desired positions. "
    ...
    if (request->velocities.size() != joint_names_.size()) {
      response->result_code = 1;
      response->message = "Received an incorrect size of desired velocities. "
                          "Command will be skipped";
      RCLCPP_ERROR(this->get_logger(), response->message.c_str());
      return;
    }
